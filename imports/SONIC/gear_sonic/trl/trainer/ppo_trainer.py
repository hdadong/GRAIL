...
class TRLPPOTrainer(PPOTrainer):  # noqa: F405
    ...
    def _init_trl(
        ...
        **kwargs,
        ...
        # Propagate safe_nan from env wrapper to actor so one CLI flag controls everything
        if getattr(self.env, "safe_nan", False):
            if hasattr(model, "safe_nan"):
                model.safe_nan = True
            if hasattr(model, "policy") and hasattr(model.policy, "safe_nan"):
                model.policy.safe_nan = True
    ...
    def load_checkpoint(
        ...
        Args:
            ...
            warm_resume: If True (implies ``resume``), restore ONLY model weights
                and optimizer state. Skip LR scheduler, env state, and trainer
                counters (global_step, max_steps, etc.) so training starts fresh
