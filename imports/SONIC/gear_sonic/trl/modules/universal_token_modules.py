...
import numpy as np
import torch
from gear_sonic.trl.modules.input_projectors import (
    GRUConv2dProjector,
    build_projector_from_config,
)
...
class UniversalTokenModule(nn.Module):
    ...
    * ``"pre_quantization_replace"`` - latent is replaced entirely by the residual.
    ...
    def __init__(
        ...
        pretrained_recon_checkpoint=None,  # Optional path: load encoder weights from recon train ckpt
        pretrained_recon_encoder_name="g1",  # Which encoders.<name>.* keys to strip and load
        **kwargs,  # noqa: ARG002
    ):
        ...
        self.pretrained_recon_checkpoint = pretrained_recon_checkpoint
        self.pretrained_recon_encoder_name = pretrained_recon_encoder_name
        ...
        logger.info(
            f"Motion Encoder and Quantizer initialized with embedding dim: "
            f"{self.token_total_dim} (num_tokens={self.max_num_tokens}, "
        ...
        self.encoder_input_projectors = nn.ModuleDict()  # per-encoder input projectors
        ...
        for encoder_name, encoder_config in self.encoders_cfg.items():
            ...
            # Input projectors: small MLPs or Conv2d heads that project specific inputs before concatenation
            input_projectors_cfg = encoder_config.get("input_projectors", {})
            if input_projectors_cfg:
                projector_dict = nn.ModuleDict()
                for feat_name, proj_cfg in input_projectors_cfg.items():
                    feat_dim = (
                    ...
                    logger.info(
                        f"Initialized input projector for {encoder_name}.{feat_name}: {desc}"
                    )
                self.encoder_input_projectors[encoder_name] = projector_dict

            # Compute input_feature_dim accounting for per-encoder projectors
            input_feature_dim = 0
        ...
        for decoder_name, decoder_config in self.decoders_cfg.items():
            ...
            logger.info(
                f"Initialized {decoder_name} decoder with input features: "
        ...
        # Optional: load encoder weights from a recon checkpoint.
        # Expects keys like `encoders.<name>.*` in the checkpoint state dict.
        if self.pretrained_recon_checkpoint is not None:
            encoder_name = self.pretrained_recon_encoder_name
            if encoder_name in self.encoders:
                ckpt = torch.load(self.pretrained_recon_checkpoint, map_location="cpu")
                state_dict = ckpt.get("model_state_dict", ckpt)
                prefix = f"encoders.{encoder_name}."
                sub_state_dict = {
                    k[len(prefix) :]: v for k, v in state_dict.items() if k.startswith(prefix)
                }
                if len(sub_state_dict) == 0:
                    raise ValueError(
                        f"No keys with prefix '{prefix}' in pretrained_recon_checkpoint="
                        f"'{self.pretrained_recon_checkpoint}'. Check pretrained_recon_encoder_name."
                    )

                target_sd = self.encoders[encoder_name].state_dict()
                load_dict = {}
                for target_key, target_param in target_sd.items():
                    src = sub_state_dict.get(target_key, None)
                    if src is not None and src.shape == target_param.shape:
                        load_dict[target_key] = src
                missing, unexpected = self.encoders[encoder_name].load_state_dict(
                    load_dict, strict=False
                )
                if missing:
                    raise ValueError(
                        f"Pretrained recon encoder '{encoder_name}' does not match RL encoder structure: "
                        f"loaded {len(load_dict)}/{len(target_sd)} params, missing_keys={list(missing)}. "
                        "Align actor_critic encoder config with recon or fix checkpoint."
                    )
                if unexpected:
                    logger.warning(
                        f"[UniversalTokenModule] Unexpected keys ignored when loading "
                        f"'{encoder_name}': {unexpected}"
                    )
                logger.info(
                    f"[UniversalTokenModule] Loaded pretrained recon encoder '{encoder_name}' params from "
                    f"'{self.pretrained_recon_checkpoint}': {len(load_dict)}/{len(target_sd)}."
                )
            else:
                logger.warning(
                    f"[UniversalTokenModule] pretrained_recon_encoder_name='{encoder_name}' "
                    f"not found in encoders. Available encoders: {list(self.encoders.keys())}"
                )
        ...
        # Height adapter: trainable MLP that produces a token-sized additive offset
        # height_map -> flatten -> MLP -> 64-dim token -> added to raw latent before FSQ
        height_adapter = kwargs.get("height_adapter", None)
        self.height_adapter_mlp = None
        if height_adapter is not None:
            height_input_dim = height_adapter["input_dim"]
            hidden_dims = height_adapter.get("hidden_dims", [512, 256])
            activation_cls = getattr(nn, height_adapter.get("activation", "SiLU"))

            layers = []
            in_dim = height_input_dim
            for h in hidden_dims:
                layers.extend([nn.Linear(in_dim, h), activation_cls()])
                in_dim = h
            # Final linear to token_total_dim (64)
            layers.append(nn.Linear(in_dim, self.token_total_dim))
            self.height_adapter_mlp = nn.Sequential(*layers)

            logger.info(
                f"Initialized height adapter: input_dim={height_input_dim}, "
                f"hidden_dims={hidden_dims}, output_dim={self.token_total_dim}"
            )
    ...
    def create_frame_and_token_masks(self, tokenizer_obs):
    ...
    def encode_single(
        ...
        Args:
            ...
            rnn_states: Optional dict of GRU hidden states keyed by
                ``"{encoder_name}.{feat_name}"``.  Updated in-place with new
                hidden states from any GRUConv2dProjector encountered.
        ...
        # Apply input projectors if configured for this encoder
        projectors = (
        ...
        for key in input_features:
            ...
            if projectors is not None and key in projectors:
                ...
                if isinstance(projector, GRUConv2dProjector):
                    # GRU projector needs hidden state
                    state_key = f"{encoder_name}.{key}"
                    hidden = rnn_states.get(state_key) if rnn_states is not None else None
                    obs, new_hidden = projector(obs, hidden=hidden)
                    if rnn_states is not None:
                        rnn_states[state_key] = new_hidden
                else:
                    obs = projector(obs)  # noqa: PLW2901
            obs_list.append(obs)

        # Handle dimension mismatch: broadcast lower-dim obs to match higher-dim ones
        # (e.g. projected depth is 3D while nonflat motion obs are 4D)
        ...
        if any(obs.ndim < max_ndim for obs in obs_list):
            ...
            for obs in obs_list:
                aligned = obs
                if aligned.ndim < max_ndim:
                    while aligned.ndim < max_ndim:
                        aligned = aligned.unsqueeze(-2)
                    expand_shape = list(aligned.shape)
                    expand_shape[-2] = temporal_dim
                    aligned = aligned.expand(*expand_shape)
                aligned_obs_list.append(aligned)
    ...
    def encode(
        ...
        Args:
            ...
            rnn_states: Optional dict of GRU hidden states (updated in-place).
            frame_mask: Optional [B*seq, max_frames] bool mask for variable frame support
        ...
        if encoder_name in self.additive_encoders:
            for additive_encoder_name in self.additive_encoders[encoder_name]:
                additive_latent = self.encode_single(
    ...
    def forward(  # noqa: D417
        ...
        **kwargs,  # noqa: ARG002
        ...
        frame_mask, token_mask = self.create_frame_and_token_masks(tokenizer_obs)
        ...
        if latent_residual is not None and latent_residual_mode in [
            ...
            height_adapter_active = (
                self.height_adapter_mlp is not None and "height_map" in input_data
            )
            if height_adapter_active:
                # Height adapter: add height token to raw latent, then quantize
                hm = input_data["height_map"]
                if isinstance(hm, dict):
                    hm = list(hm.values())[0]

                for encoder_name in self.encoders_to_iterate:
                    encoder_mask = encoder_masks[encoder_name]
                    # Get raw latent without quantization
                    latent = self.encode(
                        encoder_name,
                        tokenizer_obs,
                        encoder_mask,
                    ...
                    # Add height token to latent before FSQ
                    if latent.shape[0] > 0:
                        n_samples = latent.shape[0]
                        latent_flat = latent.view(n_samples, self.token_total_dim)  # (N, 64)

                        hm_2d = hm.reshape(batch_size * seq_len, -1)  # (B*seq, 363)
                        if encoder_mask is not None:
                            hm_2d = hm_2d[encoder_mask]
                        height_token = self.height_adapter_mlp(hm_2d)  # (N, 64)
                        height_adapter_feat = height_token  # save for aux loss
                        latent = (latent_flat + height_token).view(
                            n_samples, self.max_num_tokens, self.token_dim
                        )

                    # Quantize the adapted latent
                    if self.quantizer is not None:
                        quantized_codes, _ = self.quantizer(latent)
                        encoded_tokens[encoder_name] = quantized_codes.contiguous()
                    else:
        ...
        if compute_aux_loss:
            ...
            soma_encoder_name = "soma" if "soma" in self.encoders_to_iterate else None
            ...
            # Compute paired g1-soma latents for G1SomaLatentLoss
            if (
                soma_encoder_name is not None
                and "g1" in encoded_latents
                and "soma" in encoded_latents
            ):
                paired_g1_soma_latents = {
                    "g1": encoded_latents["g1"],
                    "soma": encoded_latents["soma"],
                }
            ...
            loss_inputs = {
                ...
                "height_adapter_feat": height_adapter_feat,
                ...
                "paired_g1_soma_latents": paired_g1_soma_latents,
                "original_g1_latents_for_reencode": original_g1_latents_for_reencode,
            }

            for loss_name, loss_func in self.aux_loss_func.items():
                result = loss_func(loss_inputs)
                if isinstance(result, dict):
                    aux_losses[loss_name] = result["loss"]
                    for k, v in result.items():
                        if k != "loss":
                            aux_losses[f"{loss_name}/{k}"] = v
                else:
                    aux_losses[loss_name] = result
    ...
    def get_all_example_input(self, batch_size=1, device="cpu"):
        ...
        for feature_name in self.tokenizer_obs_names:
            feature_dims = self.tokenizer_obs_dims[feature_name]
            end = total_feature_dim + torch.prod(torch.tensor(feature_dims)).item()
            print(  # noqa: T201
                f"{feature_name}: {feature_dims}, " f"start: {total_feature_dim} end: {end}"
            )
            total_feature_dim += torch.prod(torch.tensor(feature_dims)).item()
