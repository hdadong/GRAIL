...
def _train_breadcrumb(config, message):
    try:
        experiment_dir = Path(config.experiment_dir)
        experiment_dir.mkdir(exist_ok=True, parents=True)
        path = experiment_dir / "debug_breadcrumbs.log"
        with path.open("a", buffering=1) as f:
            f.write(f"{message}\n")
        print(f"[train_breadcrumb] {message}", flush=True)
    except Exception as exc:
        print(f"[train_breadcrumb_failed] {message}: {exc}", flush=True)
...
def create_manager_env(config, device, args_cli):
    ...
    _train_breadcrumb(config, "create_manager_env: importing ManagerBasedRLEnv")
    from isaaclab.envs import (
        ManagerBasedRLEnv,
    )

    _train_breadcrumb(config, "create_manager_env: importing ManagerEnvWrapper")
    from gear_sonic.envs.wrapper.manager_env_wrapper import ManagerEnvWrapper

    _train_breadcrumb(config, "create_manager_env: instantiate manager_env cfg")
    env_instance_cfg = custom_instantiate(config.manager_env)
    _train_breadcrumb(config, "create_manager_env: instantiated manager_env cfg")
    ...
    _train_breadcrumb(config, "create_manager_env: constructing ManagerBasedRLEnv")
    env = ManagerBasedRLEnv(
        cfg=env_instance_cfg, render_mode="rgb_array" if not args_cli.headless else None
    )
    _train_breadcrumb(config, "create_manager_env: constructed ManagerBasedRLEnv")

    _train_breadcrumb(config, "create_manager_env: wrapping env")
    env = ManagerEnvWrapper(env, env_instance_cfg.config)
    _train_breadcrumb(config, "create_manager_env: wrapped env")
    return env
...
def main(config: OmegaConf):
    ...
    if config.multi_gpu:
        ...
        # Motion sharding: each GPU loads only its slice of motions/objects.
        # Drive MotionLibBase's rank-aware slicer from accelerate's process index
        # when the top-level cfg sets ++motion_shard_by_rank=true (used by the
        # GRAIL task-general tracking sweeps bp2/bp3/g21).
        if config.get("motion_shard_by_rank", False):
            from omegaconf import open_dict

            _rank = accelerator.process_index
            _ws = accelerator.num_processes
            with open_dict(config):
                config.manager_env.commands.motion.motion_lib_cfg.motion_shard_rank = _rank
                config.manager_env.commands.motion.motion_lib_cfg.motion_shard_world_size = _ws
            print(f"[Motion Shard] Enabled: rank {_rank}/{_ws}")
    seeding(config.seed)
    ...
    _train_breadcrumb(config, "main: before create_manager_env")
    env = create_manager_env(config, device, args_cli)
    _train_breadcrumb(config, "main: after create_manager_env")
    ...
    if config.algo.config.get("use_new_actor_critic", False):
        ...
        _train_breadcrumb(config, "main: before env.reset for obs shape")
        ...
        _train_breadcrumb(config, "main: before policy instantiate")
        ...
        _train_breadcrumb(config, "main: after policy instantiate")
        ...
        if not getattr(config.algo.config, "distill_only", False):
            _train_breadcrumb(config, "main: before value model instantiate")
            ...
            _train_breadcrumb(config, "main: after value model instantiate")
    ...
    _train_breadcrumb(config, "main: callbacks instantiated")

    ################
    # Training
    ################
    _train_breadcrumb(config, "main: before trainer instantiate")
    ...
    _train_breadcrumb(config, "main: after trainer instantiate")

    # Training loop
    _train_breadcrumb(config, "main: before trainer.train")
    trainer.train()
    _train_breadcrumb(config, "main: after trainer.train")
